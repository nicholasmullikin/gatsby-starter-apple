...
const ObjViewer = () => {
  ...
  useEffect(() => {
    ...
    if (!context.current && element !== null) {
      ...
      img.onload = function textureLoaded(e) {
  ...
  useEffect(() => {
    if (context.current) {
      ...
      // reader.setResolution(coneResolution);
      renderWindow.render();
    }
  }, [coneResolution]);
          ...
          <tr>
            <td>
              <select
                ...
                style={{ width: '100%' }}
                onInput={(ev: React.ChangeEvent<HTMLSelectElement>) => setRepresentation(Number(ev.target.value))}
              >
          ...
          <tr>
            <td>
              <input
                ...
                onChange={(ev: React.ChangeEvent<HTMLInputElement>) => setConeResolution(Number(ev.target.value))}
              />
            </td>
          </tr>
        </tbody>
